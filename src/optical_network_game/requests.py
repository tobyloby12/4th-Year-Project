...
class Request:
    def __init__(self, RequestID, SourceNode, DestNode, BandWidth, timeStart, hold_time, time_limit):
...
def generateRequests(listOfNodes, numberOfRequests, req_interval, hold_time, time_limit, seed_no):
    ...
    Key parameter:
    listOfNodes: List of nodes in topology
    numberOfRequests: Number of requests to generate
    ...
    for i in range(numberOfRequests):
        ...
        #changed to 2-3 integer range from 1,2
        bandwidth = random.randint(1, 2)
        # randomising time start
        timeStart = 60 - i*req_interval
        # creating 
        request = Request(i, source, destination, bandwidth, timeStart, hold_time=hold_time, time_limit=time_limit)
